...
def generate_new_network(out_path=""):
    ...
    disaster_mode = "demand"
    dnode_list = [9, 10, 11, 12, 13, 14, 15]
    ...
    demand_nodes = generate_demand(net_struct, n_demands, q, out_path).tolist()
    
    if disaster_mode == "end":
        generate_disaster_edges_only_end(adjacency_matrix, num_dedges, out_path)
    elif disaster_mode == "nodes":
        generate_disaster_edges_only_tonodes(adjacency_matrix, num_dedges, dnode_list, out_path)
    elif disaster_mode == "demand":
        generate_disaster_edges_only_tonodes(adjacency_matrix, num_dedges, demand_nodes, out_path)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--filepath",
                        default=f"/home/jinzhao/jinzhao/xor_smt/data/supply_chain/fast_gen_net_{timestamp}",
                        help="the filename.")
    ...
    while True:
        ...
        for samples in all_baselines_samples:
            ...
            all_baselines_plans.append(saa_find_plan(sn, samples, True, 64, 25))
        ...
        for threshold in range(3,7):
            ...
            smc_outpath = f"/home/jinzhao/jinzhao/xor_smt/xor_smc/supply_chain/LOG-SPC-{timestamp}"
            smc_start_time = time.time()
            run_spc_program(smc_binary, args.filepath, smc_outpath, threshold)
